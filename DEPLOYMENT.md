...
| `BB_ADMIN_USER` / `BB_ADMIN_PASSWORD` | No | Server-side only (`api/_admin.js`) | One unrestricted operator account: sign in through the normal form (username in the email field) for no AI ceiling, no token budget, no project cap, and credit-free blueprint issuance. Needs `AUTH_SECRET`; works without KV or OAuth. `BB_ADMIN_PASSWORD_SCRYPT` (a `scrypt$…` hash, generated with `node -e "console.log(require('./api/_passwords.js').hashPassword('the password'))"`) can replace the plaintext var. **Change or revoke by editing the env vars — no code change; rotation instantly signs out existing admin sessions.** |
