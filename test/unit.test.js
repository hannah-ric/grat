...
section('prompt budget: hard ceiling with measured headroom');
{
  ...
  // 2900 → 3100 for the children's scope class (2026-07): the "ch" key with
  // its EN 1729 age-band enum, the pinned-heights/adult-loads/anchor
  // doctrine, and the regulated-product refusal list WITH regulations named
  // (toy chests 16 CFR 1250, high chairs 1231, changing tables 1235, play
  // yards 1221, gates 1239) — the refusals are the load-bearing spend:
  // children's furniture is where a model that cannot name why it refuses
  // will approximate a regulated product. The guard still catches bloat.
  ok(tokens <= 3100, `system prompt stays under the 3100-token ceiling (measured ${tokens})`);
...
section('V-05 · __proto__ in a share code leaves Object.prototype clean');
{
  ...
  for (const [what, wire] of payloads) {
    ...
    eq(Object.keys(decoded).sort(), ['bed', 'child', 'custom', 'doors', 'drawers', 'finish', 'hardware', 'joinery', 'meta', 'overall', 'seat', 'specVersion', 'structure', 'wall', 'wood'],
