...
npm run test:handcalc          # hand-arithmetic vs engine worksheet (64/64, audit asset)
...
- `src/` — one module per concern (`ai.js` transports/protocol, `spec.js`
  ...
  `classes.js` furniture-class contracts (nine artifact sets per class; seating, wall_mounted, bed, casework, childrens scope + frame_table registered), `parametric.js` templates (incl. `chair`, `wall_shelf`, `bed`), `structural.js` physics (creep, BIFMA presets,
