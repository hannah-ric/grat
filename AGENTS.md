...
npm run test:handcalc          # hand-arithmetic vs engine worksheet (28/28, audit asset)
...
- `src/` — one module per concern (`ai.js` transports/protocol, `spec.js`
  ...
  `classes.js` furniture-class contracts (nine artifact sets per class; seating, wall_mounted + frame_table registered), `parametric.js` templates (incl. `chair`), `structural.js` physics (creep, BIFMA presets,
