...
(function () {
  ...
  // Outdoor exposure (2026-08) — append only, like every enum here. Index 0
  // (interior) never rides the wire: encode() omits "ex" for interior, so
  // every pre-exposure design still encodes byte-identically.
  const EXP = ['interior', 'covered', 'exposed'];
  ...
  function encode(spec) {
    ...
    // Exposure (2026-08): only an outdoor choice rides the wire — every
    // interior design (which is every pre-exposure design) is byte-identical.
    if (spec.exposure && spec.exposure !== 'interior') {
      w.ex = ix(EXP, spec.exposure, 0);
    }
    ...
    /* Seating (2026-07): the seat section rides the wire only on a chair, so
     * every earlier design still encodes byte-identically. Flat array, mm and
     * degrees as integers/decimals; counterHeight 0 = "not given". */
    if (spec.seat && spec.meta.template === 'chair') {
  ...
  function decode(w) {
    ...
    if (Array.isArray(w.se) && spec.meta.template === 'chair') {
      ...
      spec.seat = {
        width: n(w.se[0], 430), depth: n(w.se[1], 420), height: n(w.se[2], 445),
        slopeDeg: n(w.se[3], 0), backHeight: n(w.se[4], 0), backRake: n(w.se[5], 0),
        splayDeg: n(w.se[6], 0), counterHeight: w.se[7] ? n(w.se[7], null) : null
      };
    }
  ...
  function decodePartial(w) {
     ...
     * {w,d,h,sl,bh,br,sp,ch}. "ch":0 clears the counter height. */
    if (w.se !== undefined) {
      if (Array.isArray(w.se)) {
        patch.seat = {
          width: w.se[0], depth: w.se[1], height: w.se[2], slopeDeg: w.se[3],
          backHeight: w.se[4], backRake: w.se[5], splayDeg: w.se[6],
          counterHeight: w.se[7] ? w.se[7] : null
        };
      } else if (w.se && typeof w.se === 'object') {
        patch.seat = {};
        if (w.se.w !== undefined) patch.seat.width = w.se.w;
        if (w.se.d !== undefined) patch.seat.depth = w.se.d;
        if (w.se.h !== undefined) patch.seat.height = w.se.h;
        if (w.se.sl !== undefined) patch.seat.slopeDeg = w.se.sl;
        if (w.se.bh !== undefined) patch.seat.backHeight = w.se.bh;
        if (w.se.br !== undefined) patch.seat.backRake = w.se.br;
        if (w.se.sp !== undefined) patch.seat.splayDeg = w.se.sp;
  ...
  const SCHEMA_DOC = [
    ...
    `TPL=[${TPL.join(',')}] SPC=[${SPC.join(',')}] JNT=[${JNT.join(',')}] FIN=[${FIN.join(',')}] LVL=[${LVL.join(',')}] UNITS=[${UNITS.join(',')}] FRONT=[${FRONT.join(',')}] RUN=[${RUN.join(',')}] PUL=[${PUL.join(',')}] PRIM=[${PRIM.join(',')}] SURF=[${SURF.join(',')}] GRAIN=[${GRAIN.join(',')}] STK=[${STK.join(',')}] EXP=[${EXP.join(',')}]`,
    ...
    'OUTDOOR ("ex"=EXP, omit for interior): propose the exposure WORD only — code routes everything (durable species, Type-I glue, exterior finish, stainless/galvanized fasteners, outdoor movement math). "porch"/"covered patio"=1 covered; open patio/garden/deck=2 exposed. On exposed, non-durable species are corrected to a durable one and told; interior sheet goods (plywood/MDF parts: drawers, back panels) are REFUSED exposed — no exterior sheet exists here; wall_shelf cannot be exposed (anchor math is dry-service only).',
  ...
  BB.Codec = {
    TPL, SPC, JNT, FIN, LVL, UNITS, FRONT, RUN, PUL, PRIM, SURF, GRAIN, STK, STR, HNG, EXP, AGE,
