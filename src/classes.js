...
(function () {
   ...
   * 750 N per side joint on the nominal dining chair; capacities are the
  ...
  register({
    ...
    jointRules: {
      connections: [
        { connection: 'side rail → rear post (and all seat-frame rails → legs)', required: SEAT_FRAME_ALLOWED, prohibited: ['butt_screws', 'pocket_screws', 'biscuits', 'dowels'], reason: 'the rear-tilt couple puts ≈ 750 N through each side joint (handcalc); screws top out ≈ 1.35× margin in the densest stocked species and biscuits/dowels sit at 0.8–1.35× — all below the 1.5× gate, and halved again under the cyclic factor. Mortise & tenon (or loose tenon / bolted barrel-nut) carries it at 3× in oak. This is why a screwed chair loosens in a year.' },
    ...
    hardware: [
      ...
      { id: 'kd_hardware', item: 'M6 furniture bolts + barrel nuts, 2 per rail end', when: 'joinery.frame = kd_bolt', capacity: '1800 N per joint class (JOINT_RATING kd_bolt), SG-scaled', matchedTo: 'rear-tilt couple demand (≈ 750 N nominal) with re-snug schedule in the steps' }
    ],
    assembly: {
      sequence: [
