...
- `src/porch.js` + `src/porch.css` — the landing narrative ("the porch": masthead, chapters 01–04, honesty band, build-vs-buy calculator over the real pipeline, tiered entry paths, first-run overture, scroll-driven stage). Gated to first visits (`bb.porchSeen` + `prefs4.seenOverture`); returning/share-link/deep-link arrivals bypass pre-paint; "See the intro again" lives in More. Specs: `docs/overhaul/design-language.md` §6–§8, `docs/ui/front-porch.md`.
