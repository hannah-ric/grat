...
- `api/` — all server code, CommonJS, **zero deps** (Stripe's handful of REST calls are hand-rolled in `_stripe.js` over `fetch` + `node:crypto`, exactly as `_kv.js` hand-rolls the KV REST client — the founding no-dependency rule holds), mounted identically by `serve.js` locally and auto-detected by Vercel (see `DEPLOYMENT.md`): `chat.js` (same-origin Anthropic proxy holding `ANTHROPIC_API_KEY`; owns model choice — `ANTHROPIC_MODEL`, default `claude-sonnet-5` — the token ceiling, and per-user/-IP AI metering + burst rate limit), `auth.js` (optional OAuth logins → stateless HMAC session cookies; `_session.js` is the shared signer, not an endpoint), `store.js` (optional per-user JSON document store on Upstash/Vercel KV REST, or `.data/kv.json` in dev), `billing.js` + `stripe-webhook.js` (optional Stripe subscriptions driving the Free/Pro gate; `_entitlements.js` is the entitlement/usage authority and `_stripe.js` the hand-rolled client — both shared libraries, not endpoints). Everything auth/storage/billing is optional and degrades: no env vars → the client persists to `localStorage`, no login or upgrade UI appears. AI metering is always on (signed-in by uid, anonymous by hashed IP) so the proxy key can't be burned unmetered.
- `vendor/` — Three.js + the Fraunces (display), Hanken Grotesk (body), and IBM Plex Mono (numbers) fonts, committed, inlined at build time.
...
- UI system of record: `docs/ui/brand-system.md` ("Showroom" tokens — adopted into `src/styles.css`: palette, fluid type/space scales, shape/motion signatures), `docs/ui/redesign-reports.md` (the 2026-07 Design/Plan/Build shell redesign — phase-by-phase record of what changed and how it was verified), `docs/ui/semantic-skeleton.md` (structural target for shell markup), `docs/ui/phase2-roadmap.md` (evidence-based UI backlog), `docs/ui/interaction-system.md` (3D interaction/motion design: shaders, particles, lighting, camera, direct manipulation — laws + tiered plan), `docs/ui/front-porch.md` (landing experience: the first-run Materialization overture + scroll-told porch chapters), `docs/ui/premium-refit.md` (the 2026-07 premium restraint pass: verdict capsules, ornament retirement, chrome quieting). Verdicts (`PASS`/`ADVISORY`/`FAIL`) always ship as verdict capsules with text, never color alone. The shell is three modes — Design / Plan / Build (`body[data-mode]`, `state.mode`) — with plan sub-tabs Overview/Cut/Buy/Assemble/Safety; Shop Reference opens contextually (More menu + `data-reflink` learn-links), never as a peer tab.
