...
npm run test:handcalc          # hand-arithmetic vs engine verification worksheet (25/25 — every section must agree)
...
- `test/handcalc.js` computes every physics number twice — explicit hand arithmetic vs the engine — and every section must agree (25/25 today; the number is however many sections the worksheet holds, and it only ever goes up).
