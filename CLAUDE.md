...
npm run test:handcalc          # hand-arithmetic vs engine verification worksheet (38/38 — every section must agree)
...
2. **Class contracts** (`src/classes.js`, `BB.Classes`) — a furniture class is an engineering profile, not a template: nine mandatory artifact sets (family+couplings, sourced human factors, traced load cases, joint rules with computed reasons, a failure-mode checklist wired to live check ids, capacity-matched hardware, assembly template, refusals, fixture manifest), enforced by `validateContract`/`runChecklist` in the self-test and audit suites. `frame_table` (table/desk/bench) is the retrofit proving generality; `seating` (template `chair`: dining chairs + counter/bar stools) is the first born-on-contract class and `wall_mounted` (template `wall_shelf`: French-cleat floating shelves — substrate required, unknown/drywall refused, NDS-sourced anchor math) the second, `bed` (template `bed`: knock-down platform beds — mattress size is the master dimension, species-solved slat deck against the foam-warranty gap floor, centre-rail warranty mandate, kd-bolt knock-down mandate, EN 1725 benchmark with no-US-standard disclosure, bunk/crib/murphy refused with their regulations) the third; `frame_table` also carries desk apron drawers (stiffness-shared band model) — rear-tilt joint couple, straight-rear-post doctrine (sawn/bent rear legs refused: short grain), stool↔counter coupling, class-mandated tenon-class seat-frame joinery at every level, and a benchmarked-not-certified BIFMA disclosure in every output (never a compliance claim). Spec of record: `docs/classes/`.
3. **Parametric layer** (`src/parametric.js`) — code-owned templates (table, desk — pencil drawers included, bench, bookshelf, nightstand, cabinet, chair, wall_shelf, bed) plus a novel-piece grammar: primitives (`post`, `rail`, `panel`, `slab`, `cylinder`) with an explicit connection graph, auto-grounded and centered by correction.
4. **Structural engine** (`src/structural.js`) — Wood Handbook material data, exact beam math (MOE→sag, MOR→strength at safety factor 4, Janka→surface duty only, never beam math), BIFMA load presets with published basis, ×2 creep on sustained loads, F2057/STURDY open-drawer tipping (failures make the wall anchor a mandatory BOM line), frame model for table-like pieces. Upstream of it, `spec.js` runs the geometric buildability audit (floor/envelope/overlap/joint-contact/connectivity/COG invariants) with up to three model-refinement rounds; a design that still fails is never presented.
5. **Derived layer** — `plans.js` (cut list with thickness-aware joinery allowances, BOM, assembly steps), `fasteners.js` (fastener-location/joinery-setout engine — screw positions, pilots, tenon setout snapped to chisel sizes; BOM counts must always match the drilling instructions), `packing.js` (1D/2D stock optimizer), `drafting.js` (dimensioned elevation SVGs + Blueprint Mode), `exports.js` + `gltf.js` (CSV/SketchUp/COLLADA/GLB).
6. **Display boundary** (`src/units.js`, `BB.Units`) — internal math is **always millimetres and SI**; conversion to display text happens exactly once, at render time. A raw `` `${x} mm` `` template string anywhere else is a bug. Deliberate exemption: SketchUp/CAD exports carry real millimetre geometry regardless of display mode.
...
- the `SRC` array at the top of each headless test file (`test/unit.test.js`, `audit.test.js`, `golden.test.js`, `battery.js`, `handcalc.js`, `benchmark-shaker.js`, `benchmark-chair.js`, `benchmark-bed.js`) and `api/_pipeline.js` — they load modules via `vm.runInThisContext`. Only browser-independent modules belong there (`engine.js`, `ui.js`, `store.js`, `billing.js`, `jointview.js`, `provenance.js`, `motion.js`, `porch.js` are excluded because they need DOM/Three/anime/browser globals).
...
- `test/handcalc.js` computes every physics number twice — explicit hand arithmetic vs the engine — and every section must agree (38/38 today; the number is however many sections the worksheet holds, and it only ever goes up).
- `test/fixtures/legacy/` is the migration corpus: 16 saved designs and share codes from versionless Phase 1 to current. A saved design must never fail to open, so a new `specVersion` migration needs a fixture — `unit.test.js` fails if the corpus does not cover every registered migration.
- `test/benchmark-shaker.js`, `test/benchmark-chair.js`, and `test/benchmark-bed.js` (run manually, not part of `npm test`) diff generated cut lists line by line against published plans (the Shaker nightstand canon; Ana White's "Classic Chairs Made Simple"; Ana White's "Modern Farmhouse Bed Frame"); every divergence is classified OURS-BETTER / EQUIVALENT / TRADITIONAL (`docs/audit/06-benchmark-shaker-nightstand.md`, `docs/classes/01-seating.md`, `docs/classes/04-beds.md`).
