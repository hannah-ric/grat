...
npm run test:handcalc          # hand-arithmetic vs engine verification worksheet (28/28 — every section must agree)
...
2. **Class contracts** (`src/classes.js`, `BB.Classes`) — a furniture class is an engineering profile, not a template: nine mandatory artifact sets (family+couplings, sourced human factors, traced load cases, joint rules with computed reasons, a failure-mode checklist wired to live check ids, capacity-matched hardware, assembly template, refusals, fixture manifest), enforced by `validateContract`/`runChecklist` in the self-test and audit suites. `frame_table` (table/desk/bench) is the retrofit proving generality; `seating` (template `chair`: dining chairs + counter/bar stools) is the first born-on-contract class and `wall_mounted` (template `wall_shelf`: French-cleat floating shelves — substrate required, unknown/drywall refused, NDS-sourced anchor math) the second; `frame_table` also carries desk apron drawers (stiffness-shared band model) — rear-tilt joint couple, straight-rear-post doctrine (sawn/bent rear legs refused: short grain), stool↔counter coupling, class-mandated tenon-class seat-frame joinery at every level, and a benchmarked-not-certified BIFMA disclosure in every output (never a compliance claim). Spec of record: `docs/classes/`.
3. **Parametric layer** (`src/parametric.js`) — code-owned templates (table, desk — pencil drawers included, bench, bookshelf, nightstand, cabinet, chair, wall_shelf) plus a novel-piece grammar: primitives (`post`, `rail`, `panel`, `slab`, `cylinder`) with an explicit connection graph, auto-grounded and centered by correction.
...
- the `SRC` array at the top of each headless test file (`test/unit.test.js`, `audit.test.js`, `golden.test.js`, `battery.js`, `handcalc.js`, `benchmark-shaker.js`, `benchmark-chair.js`) and `api/_pipeline.js` — they load modules via `vm.runInThisContext`. Only browser-independent modules belong there (`engine.js`, `ui.js`, `store.js`, `billing.js`, `jointview.js`, `provenance.js`, `motion.js`, `porch.js` are excluded because they need DOM/Three/anime/browser globals).
...
- `test/handcalc.js` computes every physics number twice — explicit hand arithmetic vs the engine — and every section must agree (28/28 today; the number is however many sections the worksheet holds, and it only ever goes up).
...
- `test/benchmark-shaker.js` and `test/benchmark-chair.js` (run manually, not part of `npm test`) diff generated cut lists line by line against published plans (the Shaker nightstand canon; Ana White's "Classic Chairs Made Simple"); every divergence is classified OURS-BETTER / EQUIVALENT / TRADITIONAL (`docs/audit/06-benchmark-shaker-nightstand.md`, `docs/classes/01-seating.md`).
